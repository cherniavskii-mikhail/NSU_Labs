...
void make_skeleton(Graph* graph, Graph* skl_graph, int numb_edges, int i_cnt)
{
	int count = 100;

	for (int i = 0; i < numb_edges; i++)
		if (graph->adj_list[i]->edge_start == i_cnt)	
			if (graph->adj_list[i]->edge_len < count)
			{
				count = graph->adj_list[i]->edge_len;
				i_cnt = i + 1;
			}
	...
	for (int i = 0; i < numb_edges; i++)
		if (graph->adj_list[i_cnt]->visited == false)
			make_skeleton(graph, skl_graph, numb_edges, i_cnt);

	return graph;
}

void Prim(Graph* graph, int numb_edges, int numb_vert)
{
...
int main()
{
	...
	Prim(graph, numb_edges, numb_vert);
